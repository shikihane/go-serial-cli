...
`gs` 从这里开始。串口枚举、控制台输入、进程清理、com0com 共享、日志和人工介入，都应该先在真实 Windows 机器上可靠工作，再考虑把精力放到其他平台。
...
- `gs share` worker 在 com0com 创建虚拟 COM 对后，使用 `gs` 内置的 Go bridge。
...
请手动安装驱动。`gs` 不应该静默安装内核驱动。本仓库不 vendor 或再分发 com0com 安装器。
...
`gs share` 使用 com0com 和内置 Go 字节桥创建虚拟 COM 共享。驱动安装仍然是显式操作；如果找不到 com0com 的 `setupc.exe`，`gs` 应该给出可操作错误。
...
创建虚拟 COM 对需要系统级驱动。`gs share` 使用 com0com，但不应该静默安装驱动。缺少前置条件时，应给出清晰的设置指引。
...
- `gs share` 用 com0com 创建虚拟 COM 对，并启动 worker 运行内置 Go 字节桥。
...
使用 `gs log <session>` 查看 `worker.log`，并用它诊断会话和 share worker。

当 `worker_state` 是 `stopped` 时，后台没有新的串口输出追加。`read` 和 `check` 仍然可以检查旧缓存，但它们不是实时设备读取。运行 `gs open <session> <port> -b <baud>` 可以重启 session worker。
...
`gs share` 路径会在 com0com 创建虚拟 COM 对后，使用 `gs` 内置的 Go bridge。`gs` 不捆绑或解压第三方可执行文件。
...
`gs` 尊重它可以集成的上游项目。感谢 com0com 项目让 Windows 上实用的虚拟 COM 端口工作流成为可能。
